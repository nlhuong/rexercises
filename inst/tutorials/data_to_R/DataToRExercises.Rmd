...
Now import the data into R using the `read.table()` function
...
you can wrap your answer with `knitr::kable()` function.
...
Note that the first column contains the copy of the rownames and is named 'X'.
This is because the original data in the "Credits.csv" file containt rownames,
and so the first row contained one fewer values than each subsequent ones.
This is why R generated a column name "X" for the missing column name.

Correct this double row names situation, by correctly reading data into R,
telling it where the row names are included. Check the `read.csv()` documentation
using `?read.csv`.
...
**Download** 'College.csv' file from 
http://www-bcf.usc.edu/~gareth/ISL/data.html to your 'Downloads'
directory. Then use `scan()` function to read the first 10 lines
of the data:
...
USER_CODE <- paste0('setwd("~/Downloads/");', as.character(USER_CODE$expressions))
...
Download the 'NASA_Labs_Facilities.xlsx' file to your Downloads folder:

```{r s1e3_1, exercise=TRUE}
download.file("http://open.nasa.gov/datasets/NASA_Labs_Facilities.xlsx",
              destfile = "~/Downloads/NASA_Facilities.xlsx")
```
...
Load all the objects saved in 'mydata.rdata'
and check that 'nasa_fc.df' and 'credit' objects are restored:
